...
function library:dropdown(properties)
	...
	local content_inline = library:create("Frame", {
		...
		Size = UDim2.new(0, dropdown_inline.AbsoluteSize.X, 0, 154), -- 150 + padding
		BackgroundColor3 = Color3.fromRGB(8, 8, 8),
	})
	...
	-- Auto-resize scroll area when items change
	UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		options.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 6)
	end)
...
function library:colorpicker(properties)
	...
	function cfg.set(color, alpha)
		...
		__input.Text = string.format("%d, %d, %d, %.2f",
			Color.R * 255,
			Color.G * 255,
			Color.B * 255,
			a
	...
	__input.FocusLost:Connect(function()
		...
		local r, g, b, alpha = library:convert_string_rgb(text)

		if r and g and b and alpha then
			cfg.set(rgb(r, g, b), alpha)
		end
	end)
